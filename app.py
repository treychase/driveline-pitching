...
views as the static HTML dashboard — Live delivery, Joint work, Efficiency,
Model diagnostics, and Glossary — sourcing all data directly through
...
def work_figures(sp):
    ...
    try:
        ...
        # 3D body coloured by joint work at ball release (sided by handedness),
        # with no plain stick figure — the work colour carries the figure.
        body = None
        try:
            br = jw.events.get("br")
...
<div id="hero">
  ...
  <p>Choose any of <b>{len(PITCHER_IDS)} pitchers</b> ({len(SP_TO_USER)} pitches) —
     ...
     velocity vectors · mechanical efficiency.</p>
  <div class="badges">
    ...
    with gr.Blocks(title="OBP Pitching Dashboard", theme=theme, css=CSS) as demo:
        ...
        with gr.Tabs():
            ...
            with gr.Tab("Efficiency"):
                eff_info = gr.Markdown(elem_classes="pitch-head")
